...
아래 항목에서 **확인**은 코드에 실제로 존재하는 구조·분기·중복을 뜻한다. **조건부 위험**은 제시한 응답 순서나 실패 조건에서 발생할 수 있는 문제로, 이번 작업에서 브라우저나 장애 주입으로 재현한 결과는 아니다. 우선순위는 보안 심각도와 무관하다.
...
| ID | 우선순위 | 항목 | 중심 관점 |
| --- | --- | --- | --- |
| R1 | P1 | 체크리스트 응답의 시각 기준과 데이터 조립 통일 | 정확성, DRY |
...
| 단계 | 대상 | 작업 | 상태 |
| --- | --- | --- | --- |
| 1-A | R1 | 체크리스트 시각 기준과 응답 변환 통일 | 예정 |
...
상위 1단계 상태: **예정**. 1-A·1-B·1-C가 모두 끝나야 완료로 표시한다.
...
제품 코드·설정·DB·배포 상태는 변경하지 않았다. 단위 테스트 통과는 기존 시나리오의 기준선이며 이 보고서의 조건부 위험이 없다는 증거는 아니다.
