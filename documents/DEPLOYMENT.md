...
`0015_db-indexes-checklist-retention.sql`은 체크리스트 보관 설정을 무기한으로
초기화하고 인덱스를 재구성합니다. 기존 테이블 재생성이나 기록 삭제는 없습니다.
새 Worker는 이 설정 테이블을 사용하므로 마이그레이션을 포함한 기본 배포
파이프라인을 사용합니다. 운영 보관 기간은 배포만으로 바뀌지 않습니다.

바탕 화면 중복 정렬 인덱스와 프로필 enabled 인덱스를 제거하고, 이름 오름차순
폴더 조회·외래 키 참조 탐색·기한별 정리를 지원합니다. 인덱스 추가도 포함하므로
전체 DB 용량 감소를 보장하지 않습니다. 운영 효과는 대표 쿼리의 실행 계획과
D1 `rows_read`, `rows_written`으로 확인합니다.
