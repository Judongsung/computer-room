...
- 내용이 없는 테스트용 위젯 추가·삭제
- 1024px 이상 화면의 12열 격자 편집
- 위젯 드래그, 크기 변경, 저장, 취소
- 1024px 미만 화면의 읽기 전용 1열 목록
- D1 기반 위젯 배치 조회·일괄 저장
...
오류 코드는 `constants/errors`에서 책임별로 코드·메시지·HTTP 상태를 함께 관리합니다. 서비스와 HTTP 계층은 구체 저장소 대신 역할별 인터페이스에 의존하므로 D1·R2 구현이나 UI 라이브러리를 다른 구현으로 교체할 수 있습니다. React Grid Layout 의존성은 데스크톱 격자 어댑터 한 곳에만 격리했습니다.
...
3. `npm run db:migrate:remote`로 D1 마이그레이션을 적용합니다.
